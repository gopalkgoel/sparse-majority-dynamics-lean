...
theorem eventually_weight_regime_sparse :
    ∀ θ T : ℝ, 1 / 2 < θ → θ < 1 → 1 < T →
      ∃ n₀ : ℕ, ∀ n ≥ n₀, ∀ p : ℝ, (T⁻¹ * (n : ℝ)^(-θ) < p ∧ p < T * (n : ℝ)^(-(1/2 : ℝ))) →
        0 < p ∧ p < 1 ∧ 1 ≤ Real.log (n : ℝ) ∧
          (Real.log (n : ℝ)) ^ 2 ≤ p * n ∧
          T * Real.log (n : ℝ) / Real.sqrt (p * n) ≤ 1 := by
  intro θ T _hθlo hθhi hT
  have hη : (0 : ℝ) < 1/2 := by norm_num
  have hT0 : 0 < T := by linarith
  have hnat : Tendsto (fun n : ℕ => (n : ℝ)) atTop atTop :=
    tendsto_natCast_atTop_atTop
  have hupper : Tendsto (fun n : ℕ => T * (n : ℝ) ^ (-(1/2 : ℝ))) atTop (𝓝 0) := by

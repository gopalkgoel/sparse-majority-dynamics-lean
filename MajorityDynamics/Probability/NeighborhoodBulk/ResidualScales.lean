...
theorem eventually_graph_scale_large_total (θ K : ℝ) (hθ : θ < 1) (hK : 0 < K) :
    ∀ᶠ n : ℕ in atTop, ∀ m : ℕ, GraphScaleWindow θ K n m → 2 * n ≤ m := by
...
theorem graph_scale_delete {θ K : ℝ} {n m d : ℕ} (hθ0 : 0 < θ) (hθ1 : θ < 1)
    (hK : 0 < K) (hn : 3 ≤ n) (hm : GraphScaleWindow θ K n m) (hlarge : 2 * n ≤ m)
    (hd : d ≤ n) : GraphScaleWindow θ (4 * K) (n - 1) (m - d) := by
  ...
  have hpowhi : (n : ℝ) ^ (2 - θ) ≤ 4 * ((n - 1 : ℕ) : ℝ) ^ (2 - θ) := by
    calc
      _ ≤ (2 * ((n - 1 : ℕ) : ℝ)) ^ (2 - θ) :=
        Real.rpow_le_rpow hn0.le hnN (by linarith)
      _ = (2 : ℝ) ^ (2 - θ) * ((n - 1 : ℕ) : ℝ) ^ (2 - θ) :=
...
theorem bipartite_scale_delete {θ K : ℝ} {n ell m d : ℕ} (hK : 0 < K)
    (hell : 2 ≤ ell) (hm : BipartiteScaleWindow θ K n ell m) (hlarge : 2 * n ≤ m)
    (hd : d ≤ n) : BipartiteScaleWindow θ (2 * K) n (ell - 1) (m - d) := by
  have hdm : d ≤ m := by omega
  ...
  · have he : ((m - d : ℕ) : ℝ) ≤ m := by exact_mod_cast Nat.sub_le m d
    have hnonneg : 0 ≤ K * (n : ℝ) ^ (2 - θ) := by positivity
    nlinarith [hm.2.2.2]

theorem eventually_graph_residual_scale (θ T : ℝ)
    (hθlo : 1 / 2 < θ) (hθhi : θ < 1) (hT : 1 < T) :
    ∀ᶠ n : ℕ in atTop, ∀ p : ℝ, DensityWindow θ T n p →
      ∀ (m : ℤ) (d : Fin n → ℤ), GraphInput T n p m d → ∀ v : Fin n,
        GraphScaleWindow θ (16 * T) (n - 1) (m.toNat - (d v).toNat) := by
  filter_upwards [eventually_graph_input_scale θ T hθlo hθhi hT,
    eventually_graph_scale_large_total θ (4 * T) hθhi (by linarith),
  ...
  simpa only [show 4 * (4 * T) = 16 * T by ring] using
    graph_scale_delete (by linarith) hθhi (by linarith : 0 < 4 * T) hn hs
      (hlarge m.toNat hs) hdegree

theorem eventually_bipartite_residual_scale (θ T : ℝ)
    (hθlo : 1 / 2 < θ) (hθhi : θ < 1) (hT : 1 < T) :
    ∀ᶠ n : ℕ in atTop, ∀ p : ℝ, DensityWindow θ T n p →
      ∀ (ell m : ℤ) (a : Fin ell.toNat → ℤ) (b : Fin n → ℤ),
        BipartiteInput T n p ell m a b → ∀ v : Fin ell.toNat,
          BipartiteScaleWindow θ (8 * T ^ 2) n (ell.toNat - 1) (m.toNat - (a v).toNat) := by
  filter_upwards [eventually_bipartite_input_scale θ T hθlo hθhi hT,
    eventually_graph_scale_large_total θ (4 * T ^ 2) hθhi (by positivity),
    eventually_bipartite_degree_room θ T hθlo hθhi hT] with n hscale hlarge hroom
  ...
  simpa only [show 2 * (4 * T ^ 2) = 8 * T ^ 2 by ring] using
    bipartite_scale_delete (by positivity : 0 < 4 * T ^ 2) (by linarith [hr.1]) hs

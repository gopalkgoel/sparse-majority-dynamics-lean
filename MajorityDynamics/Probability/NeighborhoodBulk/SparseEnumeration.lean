import MajorityDynamics.Probability.NeighborhoodBulk.SourceGrowth

/-! Application of the graph source with all polynomial-scale growth and
uniformity obligations discharged. No graphicality hypothesis is required. -/
noncomputable section
open Filter
open scoped Classical
namespace MajorityDynamics.Probability.NeighborhoodBulk
open MajorityDynamics.Literature.DegreeEnumeration

def graphTotals (θ K : ℝ) (n : ℕ) : Finset ℕ :=
  Finset.Icc ⌊K⁻¹ * (n : ℝ) ^ (2 - θ)⌋₊ ⌈K * (n : ℝ) ^ (2 - θ)⌉₊

theorem graphTotals_nonempty (θ K : ℝ) (hK : 1 ≤ K) (n : ℕ) : (graphTotals θ K n).Nonempty := by
  have hK0 : 0 < K := by linarith
  have hi : K⁻¹ ≤ K := by
    have : K⁻¹ ≤ 1 := (inv_le_one₀ hK0).mpr hK
    linarith
  apply Finset.nonempty_Icc.mpr
  have hh := (Nat.floor_le (show 0 ≤ K⁻¹ * (n : ℝ) ^ (2 - θ) by positivity)).trans
    ((mul_le_mul_of_nonneg_right hi (Real.rpow_nonneg (by positivity) _)).trans
      (Nat.le_ceil (K * (n : ℝ) ^ (2 - θ))))
  exact_mod_cast hh

theorem graph_scale_mem_totals {θ K : ℝ} {n m : ℕ} (hK : 0 < K)
    (h : GraphScaleWindow θ K n m) : m ∈ graphTotals θ K n := by
  apply Finset.mem_Icc.mpr
  constructor
  · have hh := (Nat.floor_le (show 0 ≤ K⁻¹ * (n : ℝ) ^ (2 - θ) by positivity)).trans h.1
    exact_mod_cast hh
  · have hh := h.2.trans (Nat.le_ceil (K * (n : ℝ) ^ (2 - θ)))
    exact_mod_cast hh

theorem eventually_graphTotals_scale (θ K : ℝ) (hθ : θ < 1) (hK : 0 < K) :
    ∀ᶠ n : ℕ in atTop, ∀ m ∈ graphTotals θ K n, GraphScaleWindow θ (2 * K) n m := by
  have hn := tendsto_natCast_atTop_atTop (R := ℝ)
  have hl := (((tendsto_rpow_atTop (by linarith : 0 < 2 - θ)).comp hn).const_mul_atTop
    (show 0 < K⁻¹ by positivity)).eventually (eventually_ge_atTop 2)
  have hu := (((tendsto_rpow_atTop (by linarith : 0 < 2 - θ)).comp hn).const_mul_atTop hK).eventually
    (eventually_ge_atTop 1)
  filter_upwards [hl, hu] with n hnlo hnhi
  simp only [Function.comp_apply] at hnlo hnhi
  intro m hm
  obtain ⟨hlo, hhi⟩ := Finset.mem_Icc.mp hm
  have hlo' : (⌊K⁻¹ * (n : ℝ) ^ (2 - θ)⌋₊ : ℝ) ≤ m := by exact_mod_cast hlo
  have hhi' : (m : ℝ) ≤ ⌈K * (n : ℝ) ^ (2 - θ)⌉₊ := by exact_mod_cast hhi
  constructor
  · have he : (2 * K)⁻¹ * (n : ℝ) ^ (2 - θ) = K⁻¹ * (n : ℝ) ^ (2 - θ) / 2 := by ring
    rw [he]
    nlinarith [Nat.lt_floor_add_one (K⁻¹ * (n : ℝ) ^ (2 - θ))]
  · have hh := Nat.ceil_lt_add_one (show 0 ≤ K * (n : ℝ) ^ (2 - θ) by positivity)
    nlinarith

theorem graph_enumeration_sparse (θ K : ℝ) (hθ0 : 0 < θ) (hθ1 : θ < 1) (hK : 1 ≤ K) :
    ∀ᶠ n : ℕ in atTop, ∀ m : ℕ, GraphScaleWindow θ K n m →
      ∀ d : Fin n → ℕ, GraphSourceData (7 / 12) n m d →
        RelativeApproximation (1 / 2) ((graphDegreeLaw (Fin n) m).real {d})
          ((graphBinomialLaw (Fin n) m).real {d} * graphCorrection m d) := by
  have hK0 : 0 < K := by linarith
  obtain ⟨μ₀, hμ₀, hsource⟩ := graph_enumeration_uniform_finite
  have hs := eventually_graphTotals_scale θ K hθ1 hK0
  have hdom : ∀ᶠ n in atTop, ∀ m ∈ graphTotals θ K n,
      m ≤ n.choose 2 ∧ graphDensity n m ≤ μ₀ ∧ 0 < graphErrorScale (7 / 12) n m := by
    filter_upwards [hs, eventually_graph_scale_domain θ (2 * K) μ₀ hθ0 (by positivity) hμ₀]
      with n hn hcap
    exact fun m hm => hcap m (hn m hm)
  have hgrowth : ∀ m : ℕ → ℕ, (∀ n, m n ∈ graphTotals θ K n) → ∀ r : ℝ, 0 < r →
      Asymptotics.IsLittleO atTop (fun n : ℕ => Real.log n ^ r / (n : ℝ))
        (fun n => graphDensity n (m n)) := by
    intro m hm r _
    apply graph_scale_log_growth θ (2 * K) hθ1 (by positivity) m ?_ r
    filter_upwards [hs] with n hn
    exact hn (m n) (hm n)
  obtain ⟨C, hC, hc⟩ := hsource (7 / 12) (by norm_num) (by norm_num)
    (graphTotals θ K) (graphTotals_nonempty θ K hK) hdom hgrowth
  filter_upwards [hc, eventually_graph_error_small θ K ((2 * C)⁻¹) hθ1 hK0 (by positivity)]
    with n hn he
  intro m hm d hd
  apply (hn m (graph_scale_mem_totals hK0 hm) d hd).mono
  have hh := mul_le_mul_of_nonneg_left (he m hm) hC.le
  have hidentity : C * (2 * C)⁻¹ = (1 / 2 : ℝ) := by field_simp
  exact hh.trans_eq hidentity

theorem graphFamily_nonempty_sparse (θ K : ℝ) (hθ0 : 0 < θ) (hθ1 : θ < 1) (hK : 1 ≤ K) :
    ∀ᶠ n : ℕ in atTop, ∀ m : ℕ, GraphScaleWindow θ K n m →
      ∀ d : Fin n → ℕ, GraphSourceData (7 / 12) n m d →
        (MajorityDynamics.Probability.FixedDegreeSampling.graphFamily d).Nonempty := by
  filter_upwards [graph_enumeration_sparse θ K hθ0 hθ1 hK] with n hn
  intro m hm d hd
  exact graphFamily_nonempty_of_enumeration d hd (by norm_num) (hn m hm d hd)

theorem eventually_graph_input_scale (θ T : ℝ)
    (hθlo : 1 / 2 < θ) (hθhi : θ < 1) (hT : 1 < T) :
    ∀ᶠ n : ℕ in atTop, ∀ p : ℝ,
      MajorityDynamics.Combinatorics.DegreeRatios.DensityWindow θ T n p →
        ∀ (m : ℤ) (d : Fin n → ℤ), GraphInput T n p m d →
          GraphScaleWindow θ (4 * T) n m.toNat := by
  filter_upwards [eventually_ge_atTop (1 : ℕ),
    eventually_graph_probability_range θ T hθlo hθhi hT,
    eventually_density_log_le_rpow θ T 2 (1 / 2) hθhi (by linarith)
      (by norm_num) (by norm_num)] with n hn hprob hsmall
  intro p hp m d hd
  have hn0 : 0 < n := by omega
  have hn0r : (0 : ℝ) < n := by exact_mod_cast hn0
  have hp0 := (hprob p hp).1
  have hx : 0 < p * n := mul_pos hp0 hn0r
  have hc := graph_input_centered hn0 hp0 hd
  have hs : 2 * Real.log n ≤ Real.sqrt (p * n) := by
    simpa only [Real.sqrt_eq_rpow] using hsmall p hp
  have hav := average_ge_half hx.le hc.1 hs
  have havhi : graphAverage n m.toNat ≤ 3 * (p * n) / 2 := by
    have hh := mul_le_mul_of_nonneg_left hs (Real.sqrt_nonneg (p * n))
    nlinarith [(abs_le.mp hc.1).2, Real.sq_sqrt hx.le]
  have hmlo : p * (n : ℝ) ^ 2 / 4 ≤ m.toNat := by
    have hh := (le_div_iff₀ hn0r).mp hav
    nlinarith
  have hmhi : (m.toNat : ℝ) ≤ 3 * p * (n : ℝ) ^ 2 / 4 := by
    have hh := (div_le_iff₀ hn0r).mp havhi
    nlinarith
  have he : (n : ℝ) ^ (-θ) * (n : ℝ) ^ 2 = (n : ℝ) ^ (2 - θ) := by
    rw [← Real.rpow_two, ← Real.rpow_add hn0r]
    congr 1
    ring
  have hlo := mul_le_mul_of_nonneg_right hp.1.le (sq_nonneg (n : ℝ))
  have hhi := mul_le_mul_of_nonneg_right hp.2.le (sq_nonneg (n : ℝ))
  rw [mul_assoc, he] at hlo hhi
  constructor
  · have hi : (4 * T)⁻¹ = T⁻¹ / 4 := by ring
    rw [hi]
    nlinarith
  · have hpow : 0 ≤ T * (n : ℝ) ^ (2 - θ) := by positivity
    nlinarith

theorem eventually_graph_input_nonempty (θ T : ℝ)
    (hθlo : 1 / 2 < θ) (hθhi : θ < 1) (hT : 1 < T) :
    ∀ᶠ n : ℕ in atTop, ∀ p : ℝ,
      MajorityDynamics.Combinatorics.DegreeRatios.DensityWindow θ T n p →
        ∀ (m : ℤ) (d : Fin n → ℤ), GraphInput T n p m d →
          (MajorityDynamics.Probability.FixedDegreeSampling.graphFamily
            (fun i => (d i).toNat)).Nonempty := by
  filter_upwards [eventually_graph_input_scale θ T hθlo hθhi hT,
    eventually_graph_source_data θ T hθlo hθhi hT,
    graphFamily_nonempty_sparse θ (4 * T) (by linarith) hθhi (by linarith)] with n hs hd hg
  intro p hp m d h
  exact hg m.toNat (hs p hp m d h) (fun i => (d i).toNat) (hd p hp m d h)

end MajorityDynamics.Probability.NeighborhoodBulk

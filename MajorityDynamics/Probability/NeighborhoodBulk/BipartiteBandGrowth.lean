...
def BipartiteBandWindow (θ η K : ℝ) (n ell m : ℕ) : Prop :=
  (n : ℝ) / K ≤ ell ∧ (ell : ℝ) ≤ K * n ∧ GraphBandWindow θ η K n m

theorem bipartite_band_bounds {θ η K : ℝ} {n ell m : ℕ} (hK : 1 ≤ K) (hn : 2 ≤ n)
    (h : BipartiteBandWindow θ η K n ell m) :
    0 < (ell : ℝ) ∧
      (K ^ 2)⁻¹ * (n : ℝ) ^ (1 - θ) ≤ leftAverage ell m ∧
      leftAverage ell m ≤ K ^ 2 * (n : ℝ) ^ (1 - η) ∧
      (K ^ 2)⁻¹ * (n : ℝ) ^ (1 - θ) ≤ rightAverage n m ∧
      bipartiteDensity ell n m ≤ K ^ 2 * (n : ℝ) ^ (-η) := by
  ...
  have hlo : (K ^ 2)⁻¹ * (n : ℝ) ^ (1 - θ) ≤ leftAverage ell m := by
    ...
    calc
      ...
      _ ≤ _ := h.2.2.1
  have hehi : (n : ℝ) ^ (1 - η) * n = (n : ℝ) ^ (2 - η) := by
    rw [← Real.rpow_add_one hn0.ne']
    congr 1
    ring
  have hhi : leftAverage ell m ≤ K ^ 2 * (n : ℝ) ^ (1 - η) := by
    apply (div_le_iff₀ hell).mpr
    calc
      _ ≤ K * (n : ℝ) ^ (2 - η) := h.2.2.2
      _ = K ^ 2 * (n : ℝ) ^ (1 - η) * ((n : ℝ) / K) := by rw [← hehi]; field_simp
      _ ≤ _ := by gcongr; exact h.1
  have hright : (K ^ 2)⁻¹ * (n : ℝ) ^ (1 - θ) ≤ rightAverage n m := by
    have hg := graph_band_bounds hK0 hn h.2.2
...
theorem bipartite_band_power_growth (θ η K : ℝ) (hθ : θ < 1) (hK : 1 ≤ K)
    (ell m : ℕ → ℕ) (hm : ∀ᶠ n in atTop, BipartiteBandWindow θ η K n (ell n) (m n)) :
...
theorem bipartite_band_log_growth (θ η K : ℝ) (hθ : θ < 1) (hK : 1 ≤ K)
    (ell m : ℕ → ℕ) (hm : ∀ᶠ n in atTop, BipartiteBandWindow θ η K n (ell n) (m n))
    (r : ℝ) (hr : 0 < r) :
    Asymptotics.IsLittleO atTop
  ...
  have hellr := (bipartite_band_bounds hK hn2 hn).1
...
theorem bipartite_band_error_third_bound {θ η K : ℝ} {n ell m : ℕ}
    (hK : 1 ≤ K) (hn : 2 ≤ n) (h : BipartiteBandWindow θ η K n ell m) :
    min (leftAverage ell m) (rightAverage n m) ^ (5 * (7 / 12 : ℝ) - 5) *
        (m : ℝ) ^ 2 / ((ell : ℝ) * n) ≤
      ((K ^ 2)⁻¹ : ℝ) ^ (-1 / 12 : ℝ) * K ^ 2 * (n : ℝ) ^ (-(1 - θ) / 12) := by
  have hK0 : 0 < K := by linarith
  have hn0 : (0 : ℝ) < n := by exact_mod_cast (by omega : 0 < n)
  have hb := bipartite_band_bounds hK hn h
  let x := leftAverage ell m
  let y := rightAverage n m
  let z := min x y
  have hlo : 0 < (K ^ 2)⁻¹ * (n : ℝ) ^ (1 - θ) := by positivity
  have hzlo : (K ^ 2)⁻¹ * (n : ℝ) ^ (1 - θ) ≤ z :=
    le_min hb.2.1 hb.2.2.2.1
  have hz : 0 < z := hlo.trans_le hzlo
  have hx : 0 ≤ x := by dsimp [x, leftAverage]; positivity
  have hy : 0 ≤ y := by dsimp [y, rightAverage]; positivity
  have hxy : x ≤ K * y := by
    dsimp [x, y, leftAverage, rightAverage]
    apply (div_le_iff₀ hb.1).mpr
    have hs : (n : ℝ) ≤ K * ell := by
      have hh := (div_le_iff₀ hK0).mp h.1
      nlinarith
    have hh := mul_le_mul_of_nonneg_left hs hy
    dsimp [y, rightAverage] at hh
    have he : (m : ℝ) / n * n = m := div_mul_cancel₀ _ hn0.ne'
    nlinarith
  have hyx : y ≤ K * x := by
    dsimp [x, y, leftAverage, rightAverage]
    apply (div_le_iff₀ hn0).mpr
    have hh := mul_le_mul_of_nonneg_left h.2.1 hx
    dsimp [x, leftAverage] at hh
    have he : (m : ℝ) / ell * ell = m := div_mul_cancel₀ _ hb.1.ne'
    nlinarith
  have hxz : x ≤ K * z := by
    dsimp [z]
    rcases le_total x y with hh | hh
    · rw [min_eq_left hh]; nlinarith
    · rw [min_eq_right hh]; exact hxy
  have hyz : y ≤ K * z := by
    dsimp [z]
    rcases le_total x y with hh | hh
    · rw [min_eq_left hh]; exact hyx
    · rw [min_eq_right hh]; nlinarith
  have hprod : x * y ≤ K ^ 2 * z ^ 2 := by
    have hh := mul_le_mul hxz hyz hy (by positivity : 0 ≤ K * z)
    nlinarith
  have he : (m : ℝ) ^ 2 / ((ell : ℝ) * n) = x * y := by
    dsimp [x, y, leftAverage, rightAverage]
    ring
  have hpow := Real.rpow_le_rpow_of_nonpos hlo hzlo (by norm_num : (-1 / 12 : ℝ) ≤ 0)
  calc
    _ = z ^ (-25 / 12 : ℝ) * (x * y) := by
      rw [show (5 * (7 / 12) - 5 : ℝ) = -25 / 12 by norm_num, mul_div_assoc, he]
    _ ≤ z ^ (-25 / 12 : ℝ) * (K ^ 2 * z ^ 2) :=
      mul_le_mul_of_nonneg_left hprod (Real.rpow_nonneg hz.le _)
    _ = K ^ 2 * z ^ (-1 / 12 : ℝ) := by
      rw [show z ^ (2 : ℕ) = z ^ (2 : ℝ) from (Real.rpow_two z).symm]
      calc
        _ = K ^ 2 * (z ^ (-25 / 12 : ℝ) * z ^ (2 : ℝ)) := by ring
        _ = _ := by rw [← Real.rpow_add hz]; norm_num
    _ ≤ K ^ 2 * ((K ^ 2)⁻¹ * (n : ℝ) ^ (1 - θ)) ^ (-1 / 12 : ℝ) :=
      mul_le_mul_of_nonneg_left hpow (sq_nonneg K)
    _ = _ := by
      rw [Real.mul_rpow (by positivity) (by positivity), ← Real.rpow_mul hn0.le]
      have hh : (1 - θ) * (-1 / 12 : ℝ) = -(1 - θ) / 12 := by ring
      rw [hh]
      ring

theorem eventually_bipartite_band_domain (θ η K μ₀ : ℝ)
    (hη : 0 < η) (hK : 1 ≤ K) (hμ₀ : 0 < μ₀) :
  ...
  have hK0 : 0 < K := by linarith
  have hu : Tendsto (fun n : ℕ => K ^ 2 * (n : ℝ) ^ (-η)) atTop (𝓝 0) := by
    simpa only [Function.comp_def, mul_zero] using ((tendsto_rpow_neg_atTop hη).comp
  ...
  intro ell m hm
  have hb := bipartite_band_bounds hK hn hm
  have hn0 : (0 : ℝ) < n := by exact_mod_cast (by omega : 0 < n)
...
theorem eventually_bipartite_band_error_small (θ η K ε : ℝ)
  ...
  have ht : Tendsto (fun n : ℕ => ((K ^ 2)⁻¹ : ℝ) ^ (-1 / 12 : ℝ) * K ^ 2 *
  ...
  have htbound := bipartite_band_error_third_bound hK hn2 hm
  unfold bipartiteErrorScale

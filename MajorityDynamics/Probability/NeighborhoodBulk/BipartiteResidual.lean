...
theorem eventually_bipartite_degree_room (θ T : ℝ)
  ...
  filter_upwards [eventually_large_parameters θ T hθlo hθhi hT, eventually_ge_atTop (4 : ℕ),
...
theorem eventually_bipartite_residual_regular (θ T : ℝ)
    (hθlo : 1 / 2 < θ) (hθhi : θ < 1) (hT : 1 < T) :
    ∀ᶠ n : ℕ in atTop, ∀ p : ℝ, DensityWindow θ T n p →
      ∀ (ell m : ℤ) (a : Fin ell.toNat → ℤ) (b : Fin n → ℤ),
  ...
  filter_upwards [eventually_large_parameters θ T hθlo hθhi hT,
    eventually_ge_atTop (3 : ℕ), eventually_bipartite_degree_room θ T hθlo hθhi hT,
    eventually_density_log_le_rpow θ T 4 (1 / 2) hθhi (by linarith) (by norm_num) (by norm_num),
    eventually_density_log_le_rpow θ T 8 (7 / 12 - 1 / 2) hθhi (by linarith) (by norm_num) (by norm_num),
    eventually_density_log_le_side_rpow θ T 4 (1 / 2) hθhi (by linarith) (by norm_num) (by norm_num),
    eventually_density_log_le_side_rpow θ T 8 (7 / 12 - 1 / 2) hθhi (by linarith) (by norm_num)
...
theorem eventually_bipartite_residual_source (θ T : ℝ)
    (hθlo : 1 / 2 < θ) (hθhi : θ < 1) (hT : 1 < T) :
    ∀ᶠ n : ℕ in atTop, ∀ p : ℝ, DensityWindow θ T n p →
      ∀ (ell m : ℤ) (a : Fin ell.toNat → ℤ) (b : Fin n → ℤ),
        BipartiteInput T n p ell m a b →
  ...
  filter_upwards [eventually_bipartite_residual_regular θ T hθlo hθhi hT] with n hn
  intro p hp ell m a b hd v S hS

...
/-! Independent lower and upper exponents for graph enumeration. The lower
exponent supplies average-degree growth; the upper exponent only forces sparsity.
No proportional-density window is assumed. -/
...
def GraphBandWindow (θ η K : ℝ) (n m : ℕ) : Prop :=
  K⁻¹ * (n : ℝ) ^ (2 - θ) ≤ m ∧ (m : ℝ) ≤ K * (n : ℝ) ^ (2 - η)

theorem graph_band_bounds {θ η K : ℝ} {n m : ℕ} (hK : 0 < K) (hn : 2 ≤ n)
    (h : GraphBandWindow θ η K n m) :
    0 < graphAverage n m ∧
      2 * K⁻¹ * (n : ℝ) ^ (1 - θ) ≤ graphAverage n m ∧
      graphAverage n m ≤ 2 * K * (n : ℝ) ^ (1 - η) ∧
      2 * K⁻¹ * (n : ℝ) ^ (-θ) ≤ graphDensity n m ∧
      graphDensity n m ≤ 4 * K * (n : ℝ) ^ (-η) := by
  ...
  have hlo : 2 * K⁻¹ * (n : ℝ) ^ (1 - θ) ≤ graphAverage n m := by
    ...
    rw [mul_assoc, he]
    linarith [h.1]
  have hhi : graphAverage n m ≤ 2 * K * (n : ℝ) ^ (1 - η) := by
  ...
  · apply (div_le_iff₀ hN).mpr
    calc
      _ ≤ 2 * K * (n : ℝ) ^ (1 - η) := hhi
      _ = 2 * K * (n : ℝ) ^ (-η) * n := by simp only [mul_assoc, hehi']
      _ ≤ 2 * K * (n : ℝ) ^ (-η) * (2 * ((n : ℝ) - 1)) := by gcongr; linarith
      _ = _ := by ring

theorem graph_band_log_growth (θ η K : ℝ) (hθ : θ < 1) (hK : 0 < K)
    (m : ℕ → ℕ) (hm : ∀ᶠ n in atTop, GraphBandWindow θ η K n (m n))
    (r : ℝ) :
  ...
  have hμ := graph_band_bounds hK hn2 hn
...
theorem eventually_graph_band_domain (θ η K μ₀ : ℝ) (hη : 0 < η) (hK : 0 < K) (hμ₀ : 0 < μ₀) :
    ∀ᶠ n : ℕ in atTop, ∀ m : ℕ, GraphBandWindow θ η K n m →
      m ≤ n.choose 2 ∧ graphDensity n m ≤ μ₀ ∧ 0 < graphErrorScale (7 / 12) n m := by
  have hu : Tendsto (fun n : ℕ => 4 * K * (n : ℝ) ^ (-η)) atTop (𝓝 0) := by
    simpa using ((tendsto_rpow_neg_atTop hη).comp
  ...
  intro m hm
  have hb := graph_band_bounds hK hn hm
  have hn0 : (0 : ℝ) < n := by exact_mod_cast (by omega : 0 < n)
...
theorem eventually_graph_band_error_small (θ η K ε : ℝ) (hθ : θ < 1) (hK : 0 < K) (hε : 0 < ε) :
    ∀ᶠ n : ℕ in atTop, ∀ m : ℕ, GraphBandWindow θ η K n m →
  ...
  intro m hm
  have hb := graph_band_bounds hK hn2 hm

import MajorityDynamics.Probability.FixedDegreeEdgeConcentration.Corrections
...
def relativeRadius (A x : ℝ) : ℝ := 2*(|A|+1)*x^((4:ℝ)/7)

theorem relativeRadius_nonneg {A x : ℝ} (hx : 0 ≤ x) : 0 ≤ relativeRadius A x := by
  unfold relativeRadius
  positivity

theorem window_le_radius {A x : ℝ} (hx : 1 ≤ x) :
    A*x^((4:ℝ)/7)+1 ≤ relativeRadius A x/2 := by
  have hp : 1 ≤ x^((4:ℝ)/7) := Real.one_le_rpow hx (by norm_num)
  have ha := mul_le_mul_of_nonneg_right (le_abs_self A) (by positivity : 0 ≤ x^((4:ℝ)/7))
  unfold relativeRadius
  nlinarith

theorem radius_sq {A x N : ℝ} (hx : 0 < x) (hN : N ≠ 0) :
    (relativeRadius A x)^2 = 4*(|A|+1)^2*(x^((1:ℝ)/7)/N)*x*N := by
  unfold relativeRadius
  rw [mul_pow, spread_power_identity hx]
  field_simp
  ring

/-- Purely numerical data derived uniformly from the original density interval. -/
structure RelativeData (N p K A : ℝ) : Prop where
  N_pos : 0 < N
  p_pos : 0 < p
  N_large : 4*K ≤ N
  degree_one : 1 ≤ p*N
  ...
  sparse : p ≤ 1/(16*K^2)
  degree_square : (p*N)^2 ≤ N
  radius_small : relativeRadius A (p*N) ≤ p*N/(4*K)
  log_degree : ∀ n : ℝ, 0 < n → n ≤ K*N → Real.log n ≤ p*N

theorem eventually_relativeData {θ T K A : ℝ}
    (hθlo : 1/2 < θ) (hθhi : θ < 1) (hT : 1 < T) (hK : 1 ≤ K) :
    ∃ N₀ : ℕ, ∀ N ≥ N₀, ∀ p : ℝ, DensityWindow θ T p N → RelativeData N p K A := by
  ...
  obtain ⟨N₂,h₂⟩ := eventually_sparse_source hθlo hθhi hT hK
  refine ⟨max N₁ N₂, ?_⟩
  intro N hN p hp
  have hw := h₁ N ((le_max_left _ _).trans hN) p hp.1 hp.2
  have hs := h₂ N ((le_max_right _ _).trans hN) p hp.1 hp.2
  refine ⟨hw.1,hw.2.1,hw.2.2.1,hs.1,(le_max_right _ _).trans hw.2.2.2.1,hw.2.2.2.2,hs.2.1,?_,hs.2.2⟩
...
theorem RelativeData.dimension_pos {N p K A n : ℝ} (hw : RelativeData N p K A)
    (hK : 1 ≤ K) (hn : N/K ≤ n) : 0 < n :=
  (div_pos hw.N_pos (by linarith)).trans_le hn

theorem RelativeData.epsilon_pos {N p K A : ℝ} (hw : RelativeData N p K A) :
    0 < epsilon N p := by
  exact div_pos (Real.rpow_pos_of_pos (mul_pos hw.p_pos hw.N_pos) _) hw.N_pos

theorem RelativeData.dimension_four {N p K A n : ℝ} (hw : RelativeData N p K A)
    (hK : 1 ≤ K) (hn : N/K ≤ n) : 4 ≤ n := by
  have hK0 : 0 < K := by linarith
  exact ((le_div_iff₀ hK0).mpr hw.N_large).trans hn

theorem RelativeData.mean_small {N p K A n D : ℝ} (hw : RelativeData N p K A)
...
theorem RelativeData.reciprocal_le {N p K A n : ℝ} (hw : RelativeData N p K A)
...
theorem RelativeData.degree_lower {N p K A c d : ℝ} (hw : RelativeData N p K A)
    (hK : 1 ≤ K) (hc : p*N/K ≤ c)
...
theorem RelativeData.radius_product {N p K A n D : ℝ} (hw : RelativeData N p K A)
...
/-- Recentring the original degree interval costs at most the explicit radius. -/
theorem centered_relativeRadius {V : Type*} [Fintype V] {d : V → ℕ} {c D A x : ℝ}
    (hx : 1 ≤ x) (hd : ∀ v, |(d v:ℝ)-c| ≤ A*x^((4:ℝ)/7)+1)
    (hav : |D-c| ≤ A*x^((4:ℝ)/7)+1) (v : V) :
    |(d v:ℝ)-D| ≤ relativeRadius A x := by
  have hr := window_le_radius (A:=A) hx
  calc
    _ ≤ |(d v:ℝ)-c| + |c-D| := abs_sub_le _ _ _
    _ = |(d v:ℝ)-c| + |D-c| := by rw [abs_sub_comm c]
    _ ≤ _ := by linarith [hd v]

/-- Exact graph main-term conversion on the original and residual windows. -/
theorem graph_approximation_relative {V : Type*} [Fintype V]
    {N n m : ℕ} {p K A : ℝ} {d : V → ℕ}
    (hK : 1 ≤ K) (hz : RelativeData N p K A) (hw : GraphWindow N n m p K A d)
...
theorem bipartite_bracket_relative {L R : Type*} [Fintype L] [Fintype R]
    {N ell n m : ℕ} {p K A : ℝ} {a : L → ℕ} {b : R → ℕ}
    (hK : 1 ≤ K) (hz : RelativeData N p K A) (hw : BipartiteWindow N ell n m p K A a b)

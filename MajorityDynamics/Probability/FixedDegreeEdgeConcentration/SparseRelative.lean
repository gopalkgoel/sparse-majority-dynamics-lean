...
import MajorityDynamics.Probability.FixedDegreeEdgeConcentration.SparseRelativeNumerics
import MajorityDynamics.Probability.FixedDegreeEdgeConcentration.SparseGraphError
import MajorityDynamics.Literature.EdgeProbabilities.SparseWindowApplications
...
theorem graph_window_relative_error_sparse {θ T K A : ℝ}
  ...
  obtain ⟨C,hC,N₁,h₁⟩ := graph_window_source_error_sparse.{u} hθlo hθhi hT hK (A:=A)
  obtain ⟨N₂,h₂⟩ := eventually_relativeData_sparse hθlo hθhi hT hK (A:=A)
  obtain ⟨N₃,h₃⟩ := eventually_graphWindow_sparse.{u} hθlo hθhi hT hK (A:=A)
  ...
  have he := graph_error_relative_sparse hz.N_pos hz.degree_one rfl hK
  ...
  rw [hwgt] at he
  have hc := graph_approximation_relative_sparse hK hz hw hg.2.1 hg.2.2 a b
  calc
...
theorem bipartite_window_relative_error_sparse {θ T K A : ℝ}
    (hθlo : 1/2 < θ) (hθhi : θ < 1) (hT : 1 < T) (hK : 1 ≤ K) :
    ∃ C : ℝ, 0 < C ∧ ∃ N₀ : ℕ, ∀ N ≥ N₀, ∀ p : ℝ,
  ...
  have hK0 : 0 < K := by linarith
  obtain ⟨C,hC,N₁,h₁⟩ := bipartite_window_source_error_sparse.{u,v} hθlo hθhi hT hK (A:=A)
  obtain ⟨N₂,h₂⟩ := eventually_relativeData_sparse hθlo hθhi hT hK (A:=A)
  obtain ⟨N₃,h₃⟩ := eventually_bipartiteWindow_sparse.{u,v} hθlo hθhi hT hK (A:=A)
  ...
  have hc := bipartite_bracket_relative_sparse hK hz hw hg.2.1 hg.2.2.1 hg.2.2.2.1 hg.2.2.2.2 i j

...
import MajorityDynamics.Probability.ConditionedBinomialFourier.Sparse
...
theorem uniform_local_clt_sparse (θ : ℝ) (hθlo : 1/2 < θ) (hθhi : θ < 1)
  ...
  obtain ⟨cw, Cw, A, cp, ce, β, _hcw, _hCw, _hA, _hcp, hce, _hβ, _hβ1,
    Nbox, hbox⟩ := ConditionedBinomialBox.uniform_component_sparse θ hθlo hθhi d hd r M hM strict T hT
  obtain ⟨C, hC, hmom⟩ := uniform_directional_moments d (U := 2*T) hce (by linarith)
  obtain ⟨Nreg, hreg⟩ := ConditionedBinomialBox.uniform_regime_sparse
    (K := 1) (ε := 1) (L := 1) d hθlo hθhi hT (by norm_num) (by norm_num) (by norm_num)
  obtain ⟨Nlow, hlow⟩ := uniform_atom_lower d θ T C hθhi hT0 hC
  obtain ⟨Nhigh, hhigh⟩ := uniform_high_frequency_sparse θ hθlo hθhi d hd r M hM strict T hT
...
theorem uniform_mean_local_clt_sparse (θ : ℝ) (hθlo : 1/2 < θ) (hθhi : θ < 1)
    (d : ℕ) (hd : 1 ≤ d) (r : ℕ) (M : Fin r → Fin d → ℤ)
    (hM : Binomial.Approximation.OrthogonalRows M) (strict : Fin r → Bool)
    (T : ℝ) (hT : 1 < T) :
    ∃ c : ℝ, 0 < c ∧ ∃ N₀ : ℕ, ∀ N ≥ N₀, ∀ p : ℝ,
  ...
  obtain ⟨c, hc, N₀, h⟩ := uniform_local_clt_sparse θ hθlo hθhi d hd r M hM strict T hT
  refine ⟨c, hc, N₀, ?_⟩
  intro N hN p hpLo hpHi η q m hηlo hηhi hq hbal hmlo hmhi

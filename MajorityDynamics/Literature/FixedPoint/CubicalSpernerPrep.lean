...
https://github.com/harfe/fixed-point-theorems-lean4/blob/770940ddf9878cf61952ed53d910b92bca841838/FixedPointTheorems/cubical_sperner_prep.lean
...
lemma ccc_fun_case_D_iff {m} I (hs : simplex SC m I) :
  ...
  have h1 : (ccc_fun SC I (Fin.last m) = Fin.last SC.n) ↔ ∀ k, I 0 k ≠ I (Fin.last m) k  := by {
    rw [Fin.ext_iff]
    change coord_change_count SC (I 0) (I (Fin.last m)) = SC.n ↔ _
    have hcard :
        (Finset.univ.filter (fun k : Fin SC.n => I 0 k ≠ I (Fin.last m) k)).card =
          Fintype.card (Fin SC.n) ↔ ∀ k, I 0 k ≠ I (Fin.last m) k := by
      rw [Finset.card_eq_iff_eq_univ, Finset.eq_univ_iff_forall]
      simp only [Finset.mem_filter, Finset.mem_univ, true_and]
    simpa only [coord_change_count, Fintype.card_fin] using hcard
  }

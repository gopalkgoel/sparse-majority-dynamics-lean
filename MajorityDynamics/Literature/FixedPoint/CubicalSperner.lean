...
https://github.com/harfe/fixed-point-theorems-lean4/blob/770940ddf9878cf61952ed53d910b92bca841838/FixedPointTheorems/cubical_sperner.lean
...
import MajorityDynamics.Literature.FixedPoint.CubicalSpernerPrep
...
theorem strong_cubical_sperner (k: ℕ ) : ∀ (SC : SpernerCube), k = SC.n →
  ...
  }
  {
    ...
    have h2 : Odd (Finset.card
        {I : Fin (k + 1) → SC2.G | complete_simplex SC2 k I}) := hind SC2 rfl
    apply Eq.mpr _ h2
    apply congrArg
    ...
    symm
    have hcomp I : complete_boundary_face SC1 (f2 I) ↔ complete_simplex SC2 k I := by {
      ...
      }
      {
        ...
        apply Iff.intro
        {
          ...
          simpa only [h6] using h4 (Fin.ofNat _ j.1)
        }
        {
          intro h4 j
    ...
    {
      intro I hI
      have hI' : complete_simplex SC2 k I :=

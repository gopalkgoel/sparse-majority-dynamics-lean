...
/-- Sufficient direction of the standard sorted Erdős–Gallai criterion.
Positive length only; empty graphs are constructed internally. -/
def ErdosGallaiCriterion : Prop :=
  ∀ (n : ℕ), 0 < n → ∀ (d : Fin n → ℕ), Antitone d → Even (total d) →
    (∀ (r : ℕ) (hr : r ≤ n), 1 ≤ r →
      (∑ i ∈ prefixSet n r hr, d i) ≤ r * (r - 1) +
        ∑ i ∈ (prefixSet n r hr)ᶜ, min r (d i)) → Graphical d

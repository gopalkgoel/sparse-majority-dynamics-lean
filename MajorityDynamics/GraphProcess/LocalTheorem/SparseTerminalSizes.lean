...
import MajorityDynamics.GraphProcess.FiberTransference.Sparse

/-! Actual terminal graph transition on the full sparse range, with no
lower bound on either new child probability. -/
...
theorem uniform_core_fiber_estimates_of_transfer_sparse {θ T φ : ℝ} (n : ℕ)
    (hθlo : 1/2 < θ) (hθhi : θ < 1) (hT : 1 < T) (hφ : 0 < φ)
    (htransfer : FiberTransference.SparseTransferTheorem.{u} θ T φ n) :
    ∃ C : ℝ, 0 < C ∧ ∃ N₀ : ℕ,
  ...
  obtain ⟨N₂, h₂⟩ := RowExactTotals.uniform_conditioned_concentration_sparse (A := 1)
    n hθlo hθhi hT hφ
...
theorem uniform_core_fiber_epsilon_sparse {θ T φ ε : ℝ} (n : ℕ)
  ...
  obtain ⟨C, _, N₁, h₁⟩ := uniform_core_fiber_estimates_of_transfer_sparse
    n hθlo hθhi hT hφ
    (FiberTransference.uniform_transfer_sparse n hθlo hθhi hT hφ)
...
def SparseTerminalSizeTransitionTheorem (θ T φ : ℝ) (n : ℕ) : Prop :=
  ∃ C : ℝ, 0 < C ∧ ∀ ε : ℝ, 0 < ε → ∃ N₀ : ℕ,
    ∀ N ≥ N₀, ∀ (V : Type u) [Fintype V], Fintype.card V = N →
    ∀ p : unitInterval, T⁻¹*(N : ℝ)^(-θ) < (p : ℝ) →
    (p : ℝ) < T * (N : ℝ)^(-(1/2 : ℝ)) →
...
theorem terminal_size_transition_sparse {θ T φ : ℝ} (n : ℕ)
    (hθlo : 1/2 < θ) (hθhi : θ < 1) (hT : 1 < T) (hφ : 0 < φ) :
    SparseTerminalSizeTransitionTheorem.{u} θ T φ n := by
  refine ⟨1, zero_lt_one, ?_⟩
  intro ε hε
  obtain ⟨N₀, h₀⟩ := uniform_core_fiber_epsilon_sparse n hθlo hθhi hT hφ hε
  obtain ⟨N₁, h₁⟩ := AdmissibleFiber.uniform_admissible_unit_sparse n hθlo hθhi hT
  refine ⟨max N₀ N₁, ?_⟩
  intro N hN V inst hcard p hlo hhi y q ha

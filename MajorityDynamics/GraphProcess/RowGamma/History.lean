import MajorityDynamics.GraphProcess.RowGamma.Moments
import MajorityDynamics.GraphProcess.RowGamma.Clipping
...
/-- This constant is fixed before the requested polynomial decay exponent. -/
def rowConstant (T φ : ℝ) : ℝ := secondMomentConstant T φ + 1

theorem rowConstant_ge_one {T φ : ℝ} (hφ : 0 < φ) : 1 ≤ rowConstant T φ := by
  unfold rowConstant
  linarith [secondMomentConstant_nonneg (T := T) hφ]

/-- The literal Gamma predicate centered at each sample's actual totals has
superpolynomial probability under the actual history-conditioned independent rows. -/
theorem uniform_history_gamma {θ T φ A : ℝ} (n : ℕ)
  ...
  obtain ⟨N₁,h₁⟩ := uniform_second_moment n hθlo hθhi hT hφ
  obtain ⟨N₂,h₂⟩ := RowConcentration.uniform_degree_tail (A := A+1) n hθlo hθhi hT hφ
  obtain ⟨N₃,h₃⟩ := Numerics.eventually_clipped_tail
    ((Fintype.card (History (n+1)) : ℝ)^2) (A+1) (sq_nonneg _)
  obtain ⟨N₄,h₄⟩ := RowConcentration.eventually_tolerance_le hθlo hθhi hT

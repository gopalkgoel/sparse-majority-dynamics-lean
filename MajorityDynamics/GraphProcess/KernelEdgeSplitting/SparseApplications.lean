...
import MajorityDynamics.GraphProcess.KernelEdgeSplitting.SparseInputs
...
theorem verified_scale_sparse {θ T φ p U : ℝ} {y : Local.CoarseData V n}
    {σ : FineState.State V n} (h : SparseVerified θ T φ p U y σ) (hT : 1 < T)
    (s : History (n+1)) :
...
theorem internal_failure_le_sparse {θ T φ p U : ℝ} {y : Local.CoarseData V n}
...
theorem cut_failure_le_sparse {θ T φ p U : ℝ} {y : Local.CoarseData V n}
    {σ : FineState.State V n} (h : SparseVerified θ T φ p U y σ) (hT : 1 < T)
  ...
  calc
    _ ≤ (FineKernel.componentLaw σ).real {F | internalSample σ s F ∈ A} := by
      ...
      exact (verified_scale_sparse h hT s).trans (lt_of_not_ge hF).le
    _ = (fixedDegreeLaw (fun v : Block σ.part s => (σ.deg v s).toNat)).real A :=
      internalSample_real σ s A
    _ ≤ _ := by
...
theorem cross_failure_le_sparse {θ T φ p U : ℝ} {y : Local.CoarseData V n}
    {σ : FineState.State V n} (h : SparseVerified θ T φ p U y σ) (hT : 1 < T)
  ...
  calc
    _ ≤ (FineKernel.componentLaw σ).real {F | crossSample σ s t F ∈ A} := by
      ...
      exact (verified_scale_sparse h hT t).trans (lt_of_not_ge hF).le
...
theorem pair_failure_le_sparse {θ T φ p U : ℝ} {y : Local.CoarseData V n}
    {σ : FineState.State V n} (h : SparseVerified θ T φ p U y σ) (hT : 1 < T)
  ...
  · subst t
    by_cases hbc : b = c
    · subst c
      exact (internal_failure_le_sparse h s b (hg s)).trans (hl s)
    · exact (cut_failure_le_sparse h hT s b c hbc (hg s)).trans (hl s)
  · exact (cross_failure_le_sparse h hT s t hst b c (hb s t)).trans (hl t)

...
theorem eventually_local_errors {θ T T1 C δ2 : ℝ}
    (hθlo : 1 / 2 < θ) (hθhi : θ < 1) (hT : 1 < T)
    (hT1 : 0 < T1) (hC : 0 ≤ C) (hδ : δ2 ≤ (1 - θ) / 8) :
    ∀ᶠ N : ℕ in atTop, ∀ p : Binomial.Probability, Density θ T N p → ∀ n : ℕ,
...
theorem eventually_log_le_pN_rpow {θ T a : ℝ}
...
theorem eventually_day_one_scales {θ T : ℝ}
    (hθlo : 1/2 < θ) (hθhi : θ < 1) (hT : 1 < T) :
    ∀ᶠ N : ℕ in atTop, ∀ p : Binomial.Probability, Density θ T N p →
  ...
  filter_upwards [eventually_basic θ T hθlo hθhi hT,
    eventually_log_le_pN_rpow hθhi (show 0 < T by linarith)

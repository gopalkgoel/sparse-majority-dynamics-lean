...
theorem band_from_global {θ η T N a p m : ℝ}
    (hη : 0 < η) (hηθ : η ≤ θ) (hθhi : θ < 1) (hT : 1 < T)
    (hN : 0 < N) (ha : N/T ≤ a) (haN : a ≤ N)
    (hp : T⁻¹*N^(-θ) < p) (hp' : p < T*N^(-η))
    (hm : p*N^2/(4*T^3) ≤ m) (hm' : m ≤ 2*T*p*N^2) :
    (4*T^4)⁻¹*a^(2-θ) ≤ m ∧ m ≤ (4*T^4)*a^(2-η) := by
  ...
  have hscale : N^(2-η) ≤ T^2*a^(2-η) := by
    calc
      _ ≤ (T*a)^(2-η) := Real.rpow_le_rpow hN.le hNa (by linarith)
      _ = T^(2-η)*a^(2-η) := Real.mul_rpow hT0.le ha0.le
      _ ≤ T^2*a^(2-η) := by
        gcongr
        simpa only [Real.rpow_two] using
          (Real.rpow_le_rpow_of_exponent_le hT.le (show 2-η ≤ (2:ℝ) by linarith))
  have hid : N^(-θ)*N^2 = N^(2-θ) := by
    rw [← Real.rpow_two, ← Real.rpow_add hN]
    congr 1
    ring
  have hid' : N^(-η)*N^2 = N^(2-η) := by
    rw [← Real.rpow_two, ← Real.rpow_add hN]
    congr 1
    ring
  have hlo := mul_le_mul_of_nonneg_right hp.le (sq_nonneg N)
  have hhi := mul_le_mul_of_nonneg_right hp'.le (sq_nonneg N)
  rw [mul_assoc, hid] at hlo
  rw [mul_assoc, hid'] at hhi
  constructor
  ...
  · have hh := mul_le_mul_of_nonneg_left hhi (show 0 ≤ 2*T by positivity)
    ...
    have hn : 0 ≤ T^4*a^(2-η) := by positivity
    nlinarith

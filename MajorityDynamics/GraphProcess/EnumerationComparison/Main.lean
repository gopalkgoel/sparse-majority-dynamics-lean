...
theorem uniform_weak_comparison {θ T : ℝ} (n : ℕ)
    (hθlo : 1/2 < θ) (hθhi : θ < 1) (hT : 1 < T) :
    WeakComparisonTheorem.{u} θ T n := by
  obtain ⟨N₀,hN₀⟩ := uniform_source hθlo hθhi hT
...
theorem uniform_strong_comparison {θ T CΓ : ℝ} (n : ℕ)
    (hθlo : 1/2 < θ) (hθhi : θ < 1) (hT : 1 < T) (hΓ : 0 < CΓ) :
    StrongComparisonTheorem.{u} θ T CΓ n := by
  obtain ⟨N₀,hN₀⟩ := uniform_source hθlo hθhi hT

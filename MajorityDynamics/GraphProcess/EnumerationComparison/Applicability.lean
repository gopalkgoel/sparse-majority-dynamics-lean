...
theorem prepared_count_bounds (y : Local.CoarseData V n) (d : RowArray.Ambient y.part)
    {T p : ℝ} (hT : 1 < T) (h : Prepared y d T p) (s t : History (n+1)) :
    p*(Fintype.card V : ℝ)^2/(2*T^3) ≤ (y.edge s t : ℝ) ∧
      (y.edge s t : ℝ) ≤ 2*T*p*(Fintype.card V : ℝ)^2 := by
  have hp := h.density_pos
  have hT0 : 0 < T := by linarith
  have hsz : 0 < (y.sizes s : ℝ) := by have := h.size_two s; linarith
  have hlo := (le_div_iff₀ hsz).mp (h.avg_lower s t)
  have hhi := (div_le_iff₀ hsz).mp (h.avg_upper s t)
  have hs := mul_le_mul_of_nonneg_left (h.size_lower s)
    (show 0 ≤ p*Fintype.card V/(2*T^2) by positivity)
  have hu := mul_le_mul_of_nonneg_left (show (y.sizes s : ℝ) ≤ Fintype.card V by
    exact_mod_cast y.sizes_le_card s) (show 0 ≤ 2*T*p*Fintype.card V by positivity)
  constructor
  · have he : p*Fintype.card V/(2*T^2)*((Fintype.card V : ℝ)/T) =
        p*(Fintype.card V : ℝ)^2/(2*T^3) := by field_simp
    rw [he] at hs
    exact hs.trans hlo
  · simpa only [pow_two, mul_assoc] using hhi.trans hu

theorem prepared_internal_scale (y : Local.CoarseData V n) (d : RowArray.Ambient y.part)
    {θ T p : ℝ} (hθlo : 1/2 < θ) (hθhi : θ < 1) (hT : 1 < T)
    (h : Prepared y d T p)
    (hlo : T⁻¹*(Fintype.card V : ℝ)^(-θ) < p)
    (hhi : p < T*(Fintype.card V : ℝ)^(-θ)) (s : History (n+1)) :
    GraphScaleWindow θ (4*T^4) (y.sizes s) (y.edge s s /2).toNat := by
  ...
  have hm' : (((y.edge s s/2).toNat : ℕ) : ℝ) ≤ 2*T*p*(Fintype.card V : ℝ)^2 := by
    ...
    linarith
  exact scale_from_global hθlo hθhi hT h.card_pos (h.size_lower s)
    (by exact_mod_cast y.sizes_le_card s) hlo hhi hm hm'

theorem prepared_cross_scale (y : Local.CoarseData V n) (d : RowArray.Ambient y.part)
    {θ T p : ℝ} (hθlo : 1/2 < θ) (hθhi : θ < 1) (hT : 1 < T)
    (h : Prepared y d T p)
    (hlo : T⁻¹*(Fintype.card V : ℝ)^(-θ) < p)
    (hhi : p < T*(Fintype.card V : ℝ)^(-θ)) (s t : History (n+1)) :
    BipartiteScaleWindow θ (4*T^4) (y.sizes t) (y.sizes s) (y.edge s t).toNat := by
  have hp := h.density_pos
  have hT0 : 0 < T := by linarith
  ...
  · apply scale_from_global hθlo hθhi hT h.card_pos (h.size_lower t)

...
/-!
# Gaussian comparison without a balance window

The balance hypothesis in the published A.2 interface is not used by the
proved comparison.  The lattice proof truncates in coordinate directions and
the Gaussian boundary estimate is uniform under translation of the half-space
thresholds.  This file records the stronger interface explicitly.
-/
...
def GaussianComparisonNoBalanceTheorem : Prop :=
...
theorem gaussian_comparison_no_balance : GaussianComparisonNoBalanceTheorem := by

...
theorem reference_child_mean_bounds_sparse (θ T : ℝ) (hθlo : 1 / 2 < θ) (hθhi : θ < 1)
  ...
  filter_upwards [eventually_ge_atTop Nr,
    ...
    eventually_small_sparse θ T hθlo hθhi hT n L,
    ...
    eventually_rpow_neg_le (sparseResponseRate θ) (v / (4 * M))
      (sparseResponseRate_pos hθhi) (by positivity),
    eventually_geometry_sparse θ T hθlo hθhi hT n ell 0 le_rfl]
  ...
  have hρ1 : Real.log (N : ℝ) ^ L / scale N p ≤ 1 :=
    (hsmall p hp hsub).2.2.1.trans hr1
  choose σ hσR hσγ hσeq hσest using
    href N hNr p hp a D ha hDlt

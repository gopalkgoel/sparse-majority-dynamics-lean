...
import MajorityDynamics.Idealized.PerturbedEvolution.SparseAdmissibilitySeparation
...
theorem admissibility_spec_sparse (θ T δ : ℝ) (hθlo : 1 / 2 < θ) (hθhi : θ < 1)
    (hT : 1 < T) (hδ : 0 < δ) (n ell : ℕ)
    (D : ℕ) (hDlt : n + 1 < D) (C : ℝ) (hC : 0 ≤ C) :
  ...
  obtain ⟨Ks, hKs, hsizes⟩ := faithful_sizes_admissible_sparse θ T δ hθlo hθhi hT hδ n ell D hDlt
  obtain ⟨Ke, hKe, hedges⟩ := faithful_edge_bounds_sparse θ T δ hθlo hθhi hT hδ n ell D hDlt
  obtain ⟨c, hc, hsep⟩ := faithful_separation_sparse θ T δ hθlo hθhi hT hδ n ell D hDlt
  ...
  intro p hp hsub a ha τ hτ hτT V inst y hcard hreg hf q hq htilt φ hcond hsplit
  have hsz := hsizes p hp hsub a ha τ hτ hτT y.integerSizes y.edge hf
  have hed := hedges p hp hsub a ha τ hτ hτT y.integerSizes y.edge hf
  have hsp := hsep p hp hsub a ha τ hτ hτT y.integerSizes y.edge hf

...
import MajorityDynamics.Idealized.PerturbedEvolution.SparseAdmissibilitySizes
...
theorem faithful_separation_sparse (θ T δ : ℝ) (hθlo : 1 / 2 < θ) (hθhi : θ < 1)
  ...
  obtain ⟨F, hF, htarget⟩ := faithful_target_sparse θ T δ hθlo hθhi hT hδ n ell D hDlt
  ...
  have hρ := (sparseTiltRate_bounds hκ hδ).1
  ...
  filter_upwards [htarget, faithful_geometry_sparse θ T δ hθlo hθhi hT hδ n ell D hDlt,
    eventually_basic_sparse θ T hθlo hθhi hT,
    eventually_small_sparse θ T hθlo hθhi hT n ell,
    eventually_rpow_neg_le (sparseResponseRate θ) 1 hκ zero_lt_one,
    eventually_rpow_neg_le (sparseResponseRate θ) (ζ / (4 * d * R)) hκ (by positivity),
    eventually_rpow_neg_le (sparseTiltRate θ δ) (ζ / (4 * d * T * F)) hρ (by positivity)]
      with N htarget hgeo hbasic hsmall hrate herr1 herr2
  intro p hp hsub a ha τ hτ hτT η e hf s r
  obtain ⟨hη, hcast, _, hsz⟩ := hgeo p hp hsub a ha τ hτ hτT η e hf
  ...
  have hbeta : betaScale N (p : ℝ) n * scale N p ≤ 1 := by
    have := (hsmall p hp hsub).2.2.2.trans hrate
  ...
  have hx : ∀ t, |x t - ν n t * μ n s t| ≤ ζ / (2 * d) := by
    intro t
    have hr := (ha.estimates n (Nat.lt_of_succ_lt hDlt)).normalized_target
    ...
    have hr' : |((a.state n).edges s t / (a.state n).sizes s -
        ...
        ζ / (4 * d) := by
      have hc := mul_le_mul (hRbound s t) ((hsmall p hp hsub).2.2.1.trans herr1)
        (by positivity : 0 ≤ Real.log (N : ℝ) ^ ell / scale N p) hR.le
      have hc' : R * (ζ / (4 * d * R)) = ζ / (4 * d) := by field_simp
      rw [hc'] at hc
      exact hr.trans hc
    have hf' := htarget p hp hsub a ha τ hτ hτT η e hf s t
  ...
  have href : ∑ t, character r.castSucc t * ((a.state n).sizes t : ℝ) = 0 :=
    Process.symmetric_imbalance_eq_zero (a.state n)
      (ha.symmetry n (Nat.lt_of_succ_lt hDlt)) r.castSucc

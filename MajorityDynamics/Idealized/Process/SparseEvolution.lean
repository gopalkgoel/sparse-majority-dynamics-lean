...
variable {n : ℕ}
theorem eventual_evolution_estimates_sparse (θ T : ℝ) (n ell L : ℕ) (K : ℝ)
    (hθ : θ < 1) (hT : 0 < T) (hK : 0 ≤ K) :
    ∃ ell' : ℕ, ell ≤ ell' ∧ ∀ᶠ N : ℕ in atTop,
      ∀ p : Probability, SparseRange θ T N p → ∀ x : State n, ∀ q : Local.Tilt n,
  ...
  filter_upwards [hlog, eventually_size_error_small_sparse θ T ε J hθ hT hε,
    density_scale_lower_power_sparse θ T 1 0 hθ hT zero_lt_one] with N hlogN hsmall ha

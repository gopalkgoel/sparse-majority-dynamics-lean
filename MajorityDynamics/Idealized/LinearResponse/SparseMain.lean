...
import MajorityDynamics.Idealized.LinearResponse.SparseGeometry
import MajorityDynamics.Idealized.LinearResponse.SparseExpansion
import MajorityDynamics.Idealized.LinearResponse.SparseReference
...
open MajorityDynamics.Binomial.Approximation (SparseRange scale tiltDifference Sizes)

/-- The complete finite-tilt response, at every small-response level of an
arbitrary fixed reference horizon. Density and the response cutoff are separate. -/
def SparseLinearResponseSpecTheorem : Prop :=
  ∀ θ : ℝ, 1/2 < θ → θ < 1 → ∀ n D : ℕ, n+1 < D →
  ...
  ∀ s : History (n+1), ResponseConclusion N p a n sizes s τ R C (sparseResponseRate θ)

set_option maxHeartbeats 12000000 in
theorem linear_response_spec_sparse : SparseLinearResponseSpecTheorem := by
  intro θ hθlo hθhi n D hDlt T R hT hR ell hell
  ...
  obtain ⟨L, R₀, hR₀, hγR, C₀, hC₀, N₁, hN₁, href⟩ :=
    reference_row_data_sparse θ T hθlo hθhi hT n ell hell
  ...
  obtain ⟨_, hrow⟩ := row_limits_sparse n
  ...
  obtain ⟨C_A, hC_A, N_A, _, hexp⟩ := row_expansions_sparse θ T₁ hθlo hθhi hT₁ n
  ...
  have hev : ∀ᶠ N : ℕ in atTop, ∀ p : Binomial.Probability, SparseRange θ T N p → ResponseSmall θ N p n →
      ∀ a : Process.Data, Process.Specification N p D ell a →
      ∀ τ : ℝ, T⁻¹ ≤ τ → τ ≤ T →
      ∀ sizes : Local.Sizes n,
        (∀ t, |(sizes t : ℝ) - ((a.state n).sizes t : ℝ)| ≤ T * sizeScale N (p : ℝ) n) →
      ∀ s : History (n + 1), ResponseConclusion N p a n sizes s τ R C (sparseResponseRate θ) := by
    filter_upwards [eventually_geometry_sparse θ T hθlo hθhi hT n ell (T + 1) (by linarith),
      eventually_basic_sparse θ T hθlo hθhi hT,
      eventually_small_sparse θ T hθlo hθhi hT n (L + L_a + 5),
      eventually_rpow_neg_le (sparseResponseRate θ) (φStar n ^ 2 / 32 / Kbig) hκ (by positivity),
      eventually_ge_atTop N₁, eventually_ge_atTop N_a, eventually_ge_atTop N_A]
      with N hgeo hbasic hsmall hΘsmall hNN₁ hNN_a hNN_A
    intro p hp hsub a hspec τ hτ₁ hτ₂ sizes hclose s
    obtain ⟨hNpos, hlog2, hbasic⟩ := hbasic
    obtain ⟨hs1, _, hp8⟩ := hbasic p hp
    obtain ⟨hTx, hgeo⟩ := hgeo p hp hsub
    have hfacts := hgeo a D hspec hDlt sizes hclose
    ...
    have hΘ0 : 0 ≤ (N : ℝ) ^ (-sparseResponseRate θ) := Real.rpow_nonneg hN0.le _
    have hΘ1 : (N : ℝ) ^ (-sparseResponseRate θ) ≤ 1 :=
      Real.rpow_le_one_of_one_le_of_nonpos (by exact_mod_cast hNpos) (by linarith)
    -- the budget at every exponent `a ≤ L + L_a + 5`
    have hbud := hsmall p hp hsub
    ...
    have hx1 : betaScale N (p : ℝ) n * Real.sqrt ((p : ℝ) * N) ≤ (N : ℝ) ^ (-sparseResponseRate θ) := by
    ...
    have hΘKbig : Kbig * (N : ℝ) ^ (-sparseResponseRate θ) ≤ φStar n ^ 2 / 32 := by
    ...
    have hpiece : ∀ c : ℝ, 0 ≤ c → c ≤ Kbig →
        c * (N : ℝ) ^ (-sparseResponseRate θ) ≤ φStar n ^ 2 / 32 := fun c _ hc =>
    ...
    obtain ⟨σr, hσrR, hσrγ, htilt, hest⟩ :=
      href N hNN₁ p hp a D hspec hDlt s
    ...
    have hρ0 : 0 ≤ Real.log (N : ℝ) ^ L / scale N p := by
      unfold scale
      positivity
    have hρ : Real.log (N : ℝ) ^ L / scale N p ≤ (N : ℝ) ^ (-sparseResponseRate θ) :=
    ...
    have hec : Kec * (N : ℝ) ^ (-sparseResponseRate θ) ≤ φStar n ^ 2 / 32 :=
      hpiece Kec hKec hKbig_ge.1
    have hecΘ : C₀ * (Real.log (N : ℝ) ^ L / scale N p) +
        K * (C₀ * (Real.log (N : ℝ) ^ L / scale N p)) ≤ Kec * (N : ℝ) ^ (-sparseResponseRate θ) := by
      rw [hKecdef]
    ...
    have hsplit_ref : ∀ b, |splitProbability (a.state n).sizes s b (a.tilt n s) -
        ν (n + 1) (append s b) / ν n s| ≤ Ker * (N : ℝ) ^ (-sparseResponseRate θ) := by
      intro b
    ...
    have herφ : Ker * (N : ℝ) ^ (-sparseResponseRate θ) ≤ φStar n / 16 :=
    ...
    have hμI : ∀ t, |cfirst (Local.trials (a.state n).sizes s) (a.tilt n s)
        (Local.historySupport (a.state n).sizes s) (center p (a.state n).sizes) t /
          ...
          ∫ x, x t ∂historyLaw n s| ≤ Kec * (N : ℝ) ^ (-sparseResponseRate θ) := by
      intro t
    ...
    have hμJ : ∀ b t, |cfirst (Local.trials (a.state n).sizes s) (a.tilt n s)
        (Local.childSupport (a.state n).sizes s b) (center p (a.state n).sizes) t /
          ...
          ∫ x, x t ∂childLaw n s b| ≤ Kec * (N : ℝ) ^ (-sparseResponseRate θ) := by
      intro b t
    ...
    have hcov : ∀ i t, |(csecond (Local.trials (a.state n).sizes s) (a.tilt n s)
        ...
        conditionalCovariance n s t i| ≤ Kec * (N : ℝ) ^ (-sparseResponseRate θ) := by
      intro i t
    ...
    have hε_a : error L_a N p (xiScale N (p : ℝ) n T) ≤
        (d * T + 2) * (N : ℝ) ^ (-sparseResponseRate θ) := by
    ...
    have hEsh : ∀ k : ℕ, k + 2 ≤ L + L_a + 5 →
        ...
        (1 + T ^ 2 * R ^ 2) * (N : ℝ) ^ (-sparseResponseRate θ) := by
      intro k hk
      ...
      have h3 : T ^ 2 * R ^ 2 * (betaScale N (p : ℝ) n * Real.sqrt ((p : ℝ) * N)) ^ 2 *
          Real.log (N : ℝ) ^ 2 * Real.log (N : ℝ) ^ k ≤
          T ^ 2 * R ^ 2 * (N : ℝ) ^ (-sparseResponseRate θ) := by
        have h4 : (betaScale N (p : ℝ) n * Real.sqrt ((p : ℝ) * N)) ^ 2 * Real.log (N : ℝ) ^ 2 *
            Real.log (N : ℝ) ^ k ≤ (N : ℝ) ^ (-sparseResponseRate θ) := by
          calc
    ...
    have hEsh' : ∀ k : ℕ, k + 2 ≤ L + L_a + 5 →
        ...
        T * (1 + T ^ 2 * R ^ 2) * (N : ℝ) ^ (-sparseResponseRate θ) := by
      intro k hk
      ...
      have hxk : T ^ 2 * R ^ 2 * (betaScale N (p : ℝ) n * Real.sqrt ((p : ℝ) * N)) ^ 2 *
          Real.log (N : ℝ) ^ 2 * Real.log (N : ℝ) ^ k ≤
          T ^ 2 * R ^ 2 * (N : ℝ) ^ (-sparseResponseRate θ) *
            (betaScale N (p : ℝ) n * Real.sqrt ((p : ℝ) * N)) := by
      ...
      have hmain : ((p : ℝ) + T ^ 2 * R ^ 2 * (betaScale N (p : ℝ) n * Real.sqrt ((p : ℝ) * N)) ^ 2 *
          Real.log (N : ℝ) ^ 2) * Real.log (N : ℝ) ^ k ≤
          (1 + T ^ 2 * R ^ 2) * (N : ℝ) ^ (-sparseResponseRate θ) *
            (betaScale N (p : ℝ) n * Real.sqrt ((p : ℝ) * N)) := by
        have h5 : Real.sqrt (p : ℝ) * Real.log (N : ℝ) ^ k *
            (betaScale N (p : ℝ) n * Real.sqrt ((p : ℝ) * N)) ≤
            (N : ℝ) ^ (-sparseResponseRate θ) * (betaScale N (p : ℝ) n * Real.sqrt ((p : ℝ) * N)) :=
          mul_le_mul_of_nonneg_right h1 hx.le
        linarith only [hpk, hxk, h5]
      calc
        _ ≤ (1 + T ^ 2 * R ^ 2) * (N : ℝ) ^ (-sparseResponseRate θ) *
    ...
    have hσblock : ∀ (σ : Row (n + 1)), (∀ t, |σ t| ≤ R) →
        ...
        ∃ E₀ E₁ : ℝ, 0 ≤ E₀ ∧ 0 ≤ E₁ ∧
          RowExpansions N p (a.state n).sizes sizes s (a.tilt n s)
            ...
            (tiltDifference p (a.state n).sizes (Local.trials (a.state n).sizes s)
              (Local.trials sizes s) (a.tilt n s) (processEffectiveTilt N p a n sizes s τ σ))
            E₀ E₁ ∧
          E₀ ≤ KeP * (N : ℝ) ^ (-sparseResponseRate θ) ∧
          E₁ / (τ * betaScale N (p : ℝ) n * ((p : ℝ) * N)) ≤ KeM * (N : ℝ) ^ (-sparseResponseRate θ) ∧
          E₀ / (τ * (betaScale N (p : ℝ) n * Real.sqrt ((p : ℝ) * N))) ≤
            KeM * (N : ℝ) ^ (-sparseResponseRate θ) := by
      intro σ hσ
      ...
      have hP'D : |historyMass sizes s (processEffectiveTilt N p a n sizes s τ σ) -
          historyMass (a.state n).sizes s (a.tilt n s)| ≤ Ku * (N : ℝ) ^ (-sparseResponseRate θ) := by
      ...
      have hsplit' : ∀ b, |splitProbability sizes s b (processEffectiveTilt N p a n sizes s τ σ) -
          splitProbability (a.state n).sizes s b (a.tilt n s)| ≤
            Kus * (N : ℝ) ^ (-sparseResponseRate θ) := by
        intro b
        ...
        have hKrK : Kr * (K * (Kg * (betaScale N (p : ℝ) n * Real.sqrt ((p : ℝ) * N)))) ≤
            Kr * (K * Kg) * (N : ℝ) ^ (-sparseResponseRate θ) := by
      ...
      have hP'J : ∀ b, |childMass sizes s b (processEffectiveTilt N p a n sizes s τ σ) -
        ...
        have h := abs_mul_sub_mul_le (historyMass sizes s (processEffectiveTilt N p a n sizes s τ σ))
          ...
          (splitProbability (a.state n).sizes s b (a.tilt n s))
          (historyMass (a.state n).sizes s (a.tilt n s) + Ku * (N : ℝ) ^ (-sparseResponseRate θ)) 1
          (by
            obtain ⟨ha, hb⟩ := abs_le.mp hP'D
        ...
        have hKuφ : Ku * (N : ℝ) ^ (-sparseResponseRate θ) ≤ φStar n / 16 :=
          (hpiece Ku hKu hKbig_ge.2.2.1).trans hφsq
        have hsp := hsplit' b
        have hfac : historyMass (a.state n).sizes s (a.tilt n s) + Ku * (N : ℝ) ^ (-sparseResponseRate θ) ≤ 2 := by
          have := hφ1
          linarith
      ...
      have hq₁lt : ∀ t, |(processEffectiveTilt N p a n sizes s τ σ t : ℝ) - p| <
        ...
        calc
          ...
          _ < T₁ * ((p : ℝ) / Real.sqrt ((p : ℝ) * N)) := mul_lt_mul_of_pos_right hT₁gt hpS
          _ = _ := by ring
      have hRexp := hexp N hNN_A p (sparseRange_enlarge hT0 hTT₁ hp) (a.state n).sizes sizes hSizes s
      ...
      · -- E₀ ≤ KeP Θ
        ...
        calc
          ...
          _ ≤ C_A * ((1 + T ^ 2 * R ^ 2) * (N : ℝ) ^ (-sparseResponseRate θ)) :=
      ...
      · -- E₁ / (b₀ V) ≤ KeM Θ
        ...
        calc
          _ ≤ C_A * (T * (1 + T ^ 2 * R ^ 2) * (N : ℝ) ^ (-sparseResponseRate θ)) :=
    ...
    have hKuφ : Ku * (N : ℝ) ^ (-sparseResponseRate θ) ≤ φStar n ^ 2 / 32 :=
      hpiece Ku hKu hKbig_ge.2.2.1
    have hKusφ : Kus * (N : ℝ) ^ (-sparseResponseRate θ) ≤ φStar n ^ 2 / 32 :=
      hpiece Kus hKus hKbig_ge.2.2.2.1
    have hKuJφ : (Ku + 2 * Kus) * (N : ℝ) ^ (-sparseResponseRate θ) ≤ φStar n ^ 2 / 32 :=
      hpiece (Ku + 2 * Kus) (by positivity) hKbig_ge.2.2.2.2.1
    have hKePφ : KeP * (N : ℝ) ^ (-sparseResponseRate θ) ≤ φStar n ^ 2 / 32 :=
    ...
    have hKec1 : Kec * (N : ℝ) ^ (-sparseResponseRate θ) ≤ 1 := hec.trans hφsq1
    ...
    · -- (i) mean response
      ...
      have hsmall : Ku * (N : ℝ) ^ (-sparseResponseRate θ) + E₀ ≤
      ...
      obtain ⟨hDen, hbound⟩ := mean_clause (historyMass (a.state n).sizes s (a.tilt n s))
        ...
        (τ * betaScale N (p : ℝ) n) ((p : ℝ) * N) R G₀ (Kec * (N : ℝ) ^ (-sparseResponseRate θ))
        (Ku * (N : ℝ) ^ (-sparseResponseRate θ)) E₀ E₁ ((G₀ + 1) * Real.sqrt ((p : ℝ) * N))
      ...
      have hbV : 0 < τ * betaScale N (p : ℝ) n * ((p : ℝ) * N) := by positivity
      have hKS : d * ((τ * betaScale N (p : ℝ) n * R) * ((G₀ + Kec * (N : ℝ) ^ (-sparseResponseRate θ)) *
          ((p : ℝ) * N))) / (τ * betaScale N (p : ℝ) n * ((p : ℝ) * N)) ≤ d * R * (G₀ + 1) := by
        rw [div_le_iff₀ hbV]
        have : G₀ + Kec * (N : ℝ) ^ (-sparseResponseRate θ) ≤ G₀ + 1 := by linarith
      ...
      have hEPEm : E₀ * ((G₀ + 1) * Real.sqrt ((p : ℝ) * N)) /
          (τ * betaScale N (p : ℝ) n * ((p : ℝ) * N)) ≤
          (G₀ + 1) * (KeM * (N : ℝ) ^ (-sparseResponseRate θ)) := by
      ...
      have hdecomp : 2 / historyMass (a.state n).sizes s (a.tilt n s) *
          (E₁ + (Ku * (N : ℝ) ^ (-sparseResponseRate θ) + E₀) *
            (d * ((τ * betaScale N (p : ℝ) n * R) * ((G₀ + Kec * (N : ℝ) ^ (-sparseResponseRate θ)) *
              ((p : ℝ) * N)))) +
          ...
          (E₁ / (τ * betaScale N (p : ℝ) n * ((p : ℝ) * N)) +
            (Ku * (N : ℝ) ^ (-sparseResponseRate θ) + E₀) *
              (d * ((τ * betaScale N (p : ℝ) n * R) * ((G₀ + Kec * (N : ℝ) ^ (-sparseResponseRate θ)) *
                ((p : ℝ) * N))) / (τ * betaScale N (p : ℝ) n * ((p : ℝ) * N))) +
            E₀ * ((G₀ + 1) * Real.sqrt ((p : ℝ) * N)) / (τ * betaScale N (p : ℝ) n * ((p : ℝ) * N)) +
            E₀ * (d * ((τ * betaScale N (p : ℝ) n * R) * ((G₀ + Kec * (N : ℝ) ^ (-sparseResponseRate θ)) *
              ((p : ℝ) * N))) / (τ * betaScale N (p : ℝ) n * ((p : ℝ) * N)))) := by
        ring
      rw [hddef] at hKS hdecomp
      rw [hdecomp]
      have hKS0 : 0 ≤ (Fintype.card (History (n + 1)) : ℝ) *
          ((τ * betaScale N (p : ℝ) n * R) * ((G₀ + Kec * (N : ℝ) ^ (-sparseResponseRate θ)) *
            ((p : ℝ) * N))) / (τ * betaScale N (p : ℝ) n * ((p : ℝ) * N)) := by positivity
      have hue0 : 0 ≤ Ku * (N : ℝ) ^ (-sparseResponseRate θ) + E₀ := by positivity
      have hue : Ku * (N : ℝ) ^ (-sparseResponseRate θ) + E₀ ≤
          (Ku + KeP) * (N : ℝ) ^ (-sparseResponseRate θ) := by linarith
      ...
      have hinner : E₁ / (τ * betaScale N (p : ℝ) n * ((p : ℝ) * N)) +
          (Ku * (N : ℝ) ^ (-sparseResponseRate θ) + E₀) *
            ((Fintype.card (History (n + 1)) : ℝ) * ((τ * betaScale N (p : ℝ) n * R) *
              ((G₀ + Kec * (N : ℝ) ^ (-sparseResponseRate θ)) * ((p : ℝ) * N))) /
              (τ * betaScale N (p : ℝ) n * ((p : ℝ) * N))) +
          E₀ * ((G₀ + 1) * Real.sqrt ((p : ℝ) * N)) / (τ * betaScale N (p : ℝ) n * ((p : ℝ) * N)) +
      ...
      have hfirstterm : (Fintype.card (History (n + 1)) : ℝ) *
          (R * (Kec * (N : ℝ) ^ (-sparseResponseRate θ))) =
          d * R * Kec * (N : ℝ) ^ (-sparseResponseRate θ) := by rw [hddef]; ring
      rw [hfirstterm]
      calc
        _ ≤ d * R * Kec * (N : ℝ) ^ (-sparseResponseRate θ) + 4 / φStar n *
            ((KeM + (Ku + KeP) * (d * R * (G₀ + 1)) + (G₀ + 1) * KeM + KeP * (d * R * (G₀ + 1))) *
              (N : ℝ) ^ (-sparseResponseRate θ)) := by linarith
        _ = Cmean * (N : ℝ) ^ (-sparseResponseRate θ) := by rw [hCmeandef]; ring
    ...
    · -- (ii) split response
      ...
      have hbound := split_clause (historyMass (a.state n).sizes s (a.tilt n s))
        ...
        (Kec * (N : ℝ) ^ (-sparseResponseRate θ)) (Ker * (N : ℝ) ^ (-sparseResponseRate θ))
        (Ku * (N : ℝ) ^ (-sparseResponseRate θ)) E₀ E₀
      ...
      have hKS : d * ((τ * betaScale N (p : ℝ) n * R) *
          (2 * (G₀ + Kec * (N : ℝ) ^ (-sparseResponseRate θ)) * Real.sqrt ((p : ℝ) * N))) /
      ...
      have hfirst2 : (Fintype.card (History (n + 1)) : ℝ) *
          (R * (2 * (Kec * (N : ℝ) ^ (-sparseResponseRate θ)))) +
          (Fintype.card (History (n + 1)) : ℝ) * (R * (2 * G₀)) *
            (Ker * (N : ℝ) ^ (-sparseResponseRate θ)) =
          (d * (R * (2 * Kec)) + d * (R * (2 * G₀)) * Ker) * (N : ℝ) ^ (-sparseResponseRate θ) := by
        rw [hddef]
        ring
      rw [hfirst2]
      calc
        _ ≤ (d * (R * (2 * Kec)) + d * (R * (2 * G₀)) * Ker) * (N : ℝ) ^ (-sparseResponseRate θ) +
            4 / φStar n * ((KeM + (Ku + KeP) * (d * (R * (2 * (G₀ + 1)))) + KeM +
              KeP * (d * (R * (2 * (G₀ + 1))))) * (N : ℝ) ^ (-sparseResponseRate θ)) := by linarith
        _ = Csplit * (N : ℝ) ^ (-sparseResponseRate θ) := by rw [hCsplitdef]; ring
    ...
    · -- (iii) child conditional mean
      ...
      have hβ : ∀ i, tiltDifference p (a.state n).sizes (Local.trials (a.state n).sizes s)
          (Local.trials sizes s) (a.tilt n s) (processEffectiveTilt N p a n sizes s τ σ) i =
          τ * betaScale N (p : ℝ) n * σ i := fun i => congrFun (hβfun σ) i
      have hsmall : (Ku + 2 * Kus) * (N : ℝ) ^ (-sparseResponseRate θ) + E₀ ≤
      ...
      obtain ⟨hDen, hbound⟩ := child_clause (childMass (a.state n).sizes s b (a.tilt n s))
        ...
        ((Ku + 2 * Kus) * (N : ℝ) ^ (-sparseResponseRate θ)) E₀ E₁ ((G₀ + 1) * Real.sqrt ((p : ℝ) * N))
      ...
      have hE₁b : E₁ ≤ KeM * (τ * betaScale N (p : ℝ) n * ((p : ℝ) * N)) := by
        ...
        have h2 : KeM * (N : ℝ) ^ (-sparseResponseRate θ) * (τ * betaScale N (p : ℝ) n * ((p : ℝ) * N)) ≤
            KeM * 1 * (τ * betaScale N (p : ℝ) n * ((p : ℝ) * N)) :=
          mul_le_mul_of_nonneg_right (mul_le_mul_of_nonneg_left hΘ1 hKeM) hbV.le
        linarith
      have hue1 : (Ku + 2 * Kus) * (N : ℝ) ^ (-sparseResponseRate θ) + E₀ ≤ 1 := by
      ...
      have hEPEm : E₀ * ((G₀ + 1) * Real.sqrt ((p : ℝ) * N)) ≤
        ...
        have hE₀τ' : E₀ ≤ KeM * (τ * (betaScale N (p : ℝ) n * Real.sqrt ((p : ℝ) * N))) := by
          have h2 : KeM * (N : ℝ) ^ (-sparseResponseRate θ) *
              (τ * (betaScale N (p : ℝ) n * Real.sqrt ((p : ℝ) * N))) ≤
      ...
      have hinner : E₁ + ((Ku + 2 * Kus) * (N : ℝ) ^ (-sparseResponseRate θ) + E₀) *
      ...
      have hinner0 : 0 ≤ E₁ + ((Ku + 2 * Kus) * (N : ℝ) ^ (-sparseResponseRate θ) + E₀) *
          (d * (R * (Q + Q₂)) * (τ * betaScale N (p : ℝ) n * ((p : ℝ) * N))) +
          E₀ * ((G₀ + 1) * Real.sqrt ((p : ℝ) * N) +

...
theorem eventually_geometry_sparse (θ T : ℝ) (hθlo : 1 / 2 < θ) (hθhi : θ < 1) (hT : 1 < T)
    (n ell : ℕ) (Kc : ℝ) (hKc : 0 ≤ Kc) :
  ...
  filter_upwards [eventually_level_sizes_sparse (n := n) θ T ell hθhi hT0,
    eventually_basic_sparse θ T hθlo hθhi hT, eventually_small_sparse θ T hθlo hθhi hT n 0,
    eventually_rpow_neg_le (sparseResponseRate θ)
  ...
  have hβ : (betaScale N (p : ℝ) n * Real.sqrt ((p : ℝ) * N)) ≤ (N : ℝ) ^ (-sparseResponseRate θ) := by
    have h := (hsmall p hp hsub).2.2.2

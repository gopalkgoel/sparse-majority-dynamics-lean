...
/-! Edge-scale control and positivity from the numerical faithful inputs. -/
...
private theorem product_error {a b x y H D : ℝ}
...
/-- A finite family admits one positive uniform absolute bound. -/
private theorem finite_bound {ι : Type*} [Fintype ι] (f : ι → ℝ) :
    ∃ A : ℝ, 1 ≤ A ∧ ∀ i, |f i| ≤ A := by
  classical
...
/-- LA3 and LA4, uniformly before `N`, density, and faithful integer arrays. -/
theorem faithful_edge_bounds (θ T δ : ℝ) (hθlo : 1 / 2 < θ) (hθhi : θ < 1)
    ...
    ∃ K : ℝ, 1 ≤ K ∧ ∀ᶠ N : ℕ in atTop,
      ...
      ∀ s t, |(e s t : ℝ) - (p : ℝ) * (η s : ℝ) * (η t : ℝ)| ≤ K * Local.edgeScale N p ∧
        0 < (e s t : ℝ) := by
  obtain ⟨A, hA, hν⟩ := finite_bound (ν n)
  obtain ⟨M, hM, hμ⟩ := finite_bound (fun st : History (n+1) × History (n+1) => μ n st.1 st.2)
  obtain ⟨E, hE, hε⟩ := finite_bound (fun s => ε n s / ν n s)
  ...
  filter_upwards [faithful_geometry θ T δ hθlo hθhi hT hδ n ell hk,
    ...
    eventually_rpow_neg_le (responseRate θ n) 1 (responseRate_pos hθlo hθhi hk) zero_lt_one,
    eventually_rpow_neg_le δ 1 hδ zero_lt_one,
    RowLimits.density_scale_lower_power θ T (32 * K / c^2 + 1) 0 hθhi hT0 (by positivity)]
    with N hgeo hbasic hsmall hpow hδpow hscale
  intro p hp a ha τ hτ hτT η e hf s t
  obtain ⟨hη, hcast, hclose, hg⟩ := hgeo p hp a ha τ hτ hτT η e hf
  ...
  have hlogS : Real.log (N : ℝ)^ell / S ≤ 1 := (hsmall p hp).2.2.1.trans hpow
  have hBS : B * S ≤ 1 := by
  ...
  have hab : ((a.state n).sizes s : ℝ) * ((a.state n).sizes t : ℝ) ≤ 4*A^2*(N : ℝ)^2 := by
    ...
    nlinarith
  have hmean := (ha.estimates n (Nat.lt_of_succ_lt (level_succ_lt_horizon hk))).edges s t

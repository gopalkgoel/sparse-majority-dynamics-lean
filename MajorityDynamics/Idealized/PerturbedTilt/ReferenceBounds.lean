...
/-! Uniform polynomial bounds for the dimensionless reference data. -/
...
theorem normalized_edge_scalar {a b u v y m S r : ℝ}
    (ha : 0 ≤ a) (hb : 0 ≤ b) (hu : 0 ≤ u) (hv : 0 ≤ v)
    (haU : a ≤ 2 * u) (hbU : b ≤ 2 * v) (hS : 0 < S) (hr : 0 ≤ r)
    (hir : 1 / S ≤ r) (has : |a - u| ≤ r) (hbs : |b - v| ≤ r)
    (hy : |y - a * b * (1 + m / S)| ≤ a * b * r) :
    |y - u * v| ≤ (4 * u * v * (1 + |m|) + 2 * u + v) * r := by
  have hab : a * b ≤ 4 * u * v := by nlinarith [mul_le_mul haU hbU hb (by positivity)]
  have hm : |m / S| ≤ |m| * r := by
    rw [abs_div, abs_of_pos hS, div_eq_mul_one_div]
    exact mul_le_mul_of_nonneg_left hir (abs_nonneg _)
  have h1 : |y - a * b| ≤ a * b * (1 + |m|) * r := by
    have hid : y - a * b = (y - a * b * (1 + m / S)) + a * b * (m / S) := by ring
    rw [hid]
    have := abs_add_le (y - a * b * (1 + m / S)) (a * b * (m / S))
    have hprod : |a * b * (m / S)| ≤ a * b * (|m| * r) := by
      rw [abs_mul, abs_of_nonneg (mul_nonneg ha hb)]
      exact mul_le_mul_of_nonneg_left hm (mul_nonneg ha hb)
    nlinarith
  have h2 : |a * b - u * v| ≤ (2 * u + v) * r := by
    have hid : a * b - u * v = a * (b - v) + (a - u) * v := by ring
    rw [hid]
    have hprod1 : |a * (b - v)| ≤ a * r := by
      rw [abs_mul, abs_of_nonneg ha]
      exact mul_le_mul_of_nonneg_left hbs ha
    have hprod2 : |(a - u) * v| ≤ r * v := by
      rw [abs_mul, abs_of_nonneg hv]
      exact mul_le_mul_of_nonneg_right has hv
    have := abs_add_le (a * (b - v)) ((a - u) * v)
    nlinarith [mul_le_mul_of_nonneg_right haU hr]
  have h3 := abs_sub_le y (a * b) (u * v)
  have h4 := mul_le_mul_of_nonneg_right hab (mul_nonneg (by positivity : 0 ≤ 1 + |m|) hr)
  nlinarith

theorem reference_coordinates (θ T : ℝ) (hθlo : 1 / 2 < θ) (hθhi : θ < 1) (hT : 1 < T)
    (n ell : ℕ) (hk : (n : ℝ) + 1 < 1 / (1 - θ)) :
    ∃ K : ℝ, 1 ≤ K ∧ ∀ᶠ N : ℕ in atTop,
      ∀ p : Binomial.Probability, Density θ T N p →
      ∀ x : Process.State n, Process.LevelEstimates N p ell x →
      (∀ s, |(x.sizes s : ℝ) / N - ν n s| ≤ K * (N : ℝ) ^ (-responseRate θ n)) ∧
      (∀ s t, |x.edges s t / ((p : ℝ) * (N : ℝ) ^ 2) - ν n s * ν n t| ≤
        K * (N : ℝ) ^ (-responseRate θ n)) := by
  classical
  ...
  refine ⟨fun s => (hx.relative_sizes hN s).trans (herr.trans (by nlinarith [Real.rpow_nonneg (Nat.cast_nonneg N) (-responseRate θ n)])), ?_⟩
  intro s t

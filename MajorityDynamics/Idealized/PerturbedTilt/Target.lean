...
def tiltRate (θ δ : ℝ) (n : ℕ) : ℝ := min (responseRate θ n) δ / 2

theorem tiltRate_bounds {θ δ : ℝ} {n : ℕ} (hκ : 0 < responseRate θ n) (hδ : 0 < δ) :
    0 < tiltRate θ δ n ∧ tiltRate θ δ n < responseRate θ n ∧ tiltRate θ δ n < δ := by
  have h := lt_min hκ hδ
  have h1 := min_le_left (responseRate θ n) δ
  have h2 := min_le_right (responseRate θ n) δ
  unfold tiltRate
  constructor
  · positivity
...
theorem faithful_target (θ T δ : ℝ) (hθlo : 1 / 2 < θ) (hθhi : θ < 1)
    ...
    ∃ K : ℝ, 0 < K ∧ ∀ᶠ N : ℕ in atTop,
      ...
      ∀ s t, |edgeTarget N (p : ℝ) a τ η e s t - ε n t| ≤ K * (N : ℝ) ^ (-tiltRate θ δ n) := by
  classical
  have hT0 : 0 < T := by linarith
  have hκ := responseRate_pos hθlo hθhi hk
  obtain ⟨hρ, hρκ, hρδ⟩ := tiltRate_bounds hκ hδ
  ...
  obtain ⟨Kr, hKr, href⟩ := reference_coordinates θ T hθlo hθhi hT n ell hk
  refine ⟨K₀, hK₀, ?_⟩
  filter_upwards [href, faithful_geometry θ T δ hθlo hθhi hT hδ n ell hk,
    ...
    eventually_poly_log_le Kr (responseRate θ n) (tiltRate θ δ n) 0 (by linarith) hρκ,
    eventually_poly_log_le T (responseRate θ n) (tiltRate θ δ n) 0 hT0.le hρκ,
    eventually_poly_log_le (T ^ 2) δ (tiltRate θ δ n) 0 (sq_nonneg _) hρδ,
    eventually_rpow_neg_le (tiltRate θ δ n) (a₀ / 2) hρ (half_pos ha₀)]
  ...
  obtain ⟨hη, _, _, hgf⟩ := hgeo p hp b hb τ hτ hτT η e hf
  ...
  have hc := href p hp (b.state n) (hb.estimates n (Nat.lt_of_succ_lt (level_succ_lt_horizon hk)))
  ...
  have hedge : |((e s t : ℝ) - (b.state n).edges s t *
    ...
    ring
  have hβsmall : B ≤ (N : ℝ) ^ (-responseRate θ n) := by
    have hh := (hsmall p hp).2.2.2
  ...
  let x : Fin 5 → ℝ := ![((b.state n).sizes s : ℝ) / N,
    ...
    ((e s t : ℝ) - (b.state n).edges s t * (1 + τ * B * (ε n s / ν n s + ε n t / ν n t))) /
      (τ * B * (p : ℝ) * (N : ℝ) ^ 2)]
  have hx : ∀ i, |x i - targetPoint (ν n s) (ν n t) i| ≤ (N : ℝ) ^ (-tiltRate θ δ n) := by
    intro i
    fin_cases i
  ...
  have hc' := hctl (s,t) ((N : ℝ) ^ (-tiltRate θ δ n))
